-- ymm_summary: zelfde logica als Python (_ymm_summary), opgeslagen bij XML-sync.
-- Projection gebruikt dit veld i.p.v. vereenvoudigde SQL-string.

alter table public.canonical_product_fits_on
  add column if not exists ymm_summary text;

comment on column public.canonical_product_fits_on.ymm_summary is
  'Metafields-stijl samenvatting uit Python (per OEM rijk, join met | bij cross-brand).';
...
begin
  ...
  upserted as (
    ...
    select
      ...
      nullif(trim(sr.ymm_summary), '') as ymm_summary,
      a.source_synced_at,
      now() as updated_at
    from agg a
    join source_rows sr on sr.shopify_product_id = a.shopify_product_id
    left join sku_one sk on sk.shopify_product_id = a.shopify_product_id
