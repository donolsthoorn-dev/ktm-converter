-- Gebruik pg_net _http_collect_response (wacht tot antwoord) i.p.v. handmatige poll met korte timeout.
-- Controleer ook of de pg_net background worker draait.

create or replace function public.await_net_http_response(
  p_request_id bigint,
  p_max_wait_ms integer default 120000,
...
begin
  ...
  -- Wacht tot pg_net klaar is (geen vaste 10s-cap; worker-timeout komt uit http_post).
  v_result := net._http_collect_response(p_request_id, false);

  if v_result.status::text = 'SUCCESS' and v_result.response is not null then
    v_code := (v_result.response).status_code;
    v_body := coalesce((v_result.response).body, '');
    return query select v_code, left(v_body, 4000), false;
    return;
  end if;
  ...
  select
    null::integer,
    left(coalesce(v_result.message, 'pg_net collect error'), 4000),
    true;
end;
$$;
